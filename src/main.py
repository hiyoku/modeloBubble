...
for J in range(0, JMAX):
    ...
    CO[J] = 5.29E+08 * np.exp(-(Z1 - 335.0) / HO)  # Oxigeno atomico    [cm-3]
    CO2[J] = 2.84E+06 * np.exp(-(Z1 - 335.0) / HO2)  # Oxigeno molecular [cm3]
    CN2[J] = 1.05E+08 * np.exp(-(Z1 - 335.0) / HN2)  # Nitrogeno molecular[cm3]
    BETA[J] = (RK1 * CO2[J]) + (RK2 * CN2[J])  # Recombinacion [1/s]

    # Colisões
    CFO[J] = 4.45E-11 * CO[J] * np.sqrt(TN) * np.power((1.04 - 0.067 * np.log10(TN)), 2.) + 6.64E-10 * CO2[J] + 6.82E-10 * CN2[J]
...
for M in range(0, 1):
    ...
    COSLAT = np.cos(PHI)
    SENLAT = np.sin(PHI)
    COSDIP = COSLAT / np.sqrt(1.0 + 3. * np.power(SENLAT, 2.0))
    SENDIP = 2. * SENLAT / np.sqrt(1. + 3. * np.power(SENLAT, 2.0))
    F1 = np.power(COSLAT, 3.0) / np.sqrt(1.0 + 3. * np.power(SENLAT, 2.0))   # to  zonal electric
    F2 = np.power(COSLAT, 3.0)                                 # to vertical electric
    F3 = np.sqrt(1. + 3. * np.power(SENLAT, 2.))
    FU = 1.0 / (0.7 + 0.4 * np.exp(np.power(-(XLAT), 2.0) / 200.) - 0.035)
